...
app.get("/login/if", async (req, res) => {
  if (req.cookies.massiropass !== "ok") {
    res.render("login", {
      error: "ログインしていません。もう一度ログインして下さい",
    });
  } else {
    return res.redirect("/");
  }
...
app.get("/des/:id", async (req, res) => {
  ...
  try {
    ...
    const channnelIdMatch = html.match(
      /"browseEndpoint":\{.*?"browseId":"(.*?)"/
    );
    const channelImageMatch = html.match(
    ...
    const videoDes = descriptionMatch
      ? descriptionMatch[1].replace(/\\n/g, "\n")
      : "取得できませんでした";
    const videoViews = viewsMatch
      ? viewsMatch[1]
      : "再生回数を取得できませんでした";
    const channelId = channnelIdMatch
      ? channnelIdMatch[1]
      : "取得できませんでした";
    const channelImage = channelImageMatch
      ? channelImageMatch[1]
      : "取得できませんでした";
    const channelName = channelNameMatch
      ? channelNameMatch[1]
      : "取得できませんでした";
  ...
  try {
    ...
    try {
      ...
      const descriptionMatch = html.match(
        /"attributedDescriptionBodyText":\{.*?"content":"(.*?)","commandRuns/
      );
      const viewsMatch = html.match(/"views":\{.*?"simpleText":"(.*?)"/);
      const channelImageMatch = html.match(
        /"channelThumbnail":\{.*?"url":"(.*?)"/
      );
      const channelNameMatch = html.match(
        /"channel":\{.*?"simpleText":"(.*?)"/
      );
      const channnelIdMatch = html.match(
        /"browseEndpoint":\{.*?"browseId":"(.*?)"/
      ...
      videoTitle = titleMatch
        ? titleMatch[1]
        : "タイトルを取得できませんでした";
      videoDes = descriptionMatch
        ? descriptionMatch[1].replace(/\\n/g, "\n")
        : "概要を取得できませんでした";
      videoViews = viewsMatch
        ? viewsMatch[1]
        : "再生回数を取得できませんでした";
      channelImage = channelImageMatch
        ? channelImageMatch[1]
        : "取得できませんでした";
...
app.get("/www/:id", async (req, res) => {
  ...
  try {
    ...
    let streamUrl = audioStreams
      .filter(
        (stream) => stream.container === "mp4" && stream.resolution === "1080p"
      )
    ...
    if (!streamUrl) {
      let streamUrl = audioStreams
        .filter(
          (stream) => stream.container === "mp4" && stream.resolution === "720p"
        )
        .map((stream) => stream.url)[0];
    }
...
app.get("/sage", async (req, res) => {
  ...
  const url = `https://www.google.com/complete/search?client=youtube&hl=ja&ds=yt&q=${encodeURIComponent(
    query
  )}`;
  ...
  try {
    ...
    const suggestions = Array.isArray(response.data[1][0])
      ? response.data[1][0]
      : [];
      ...
      .get(streamUrl, (streamResponse) => {
        if (streamResponse.statusCode !== 200) {
          res
            ...
            .send(
              `Failed to download video. Status code: ${streamResponse.statusCode}`
            );
          return;
        }
        ...
        res.setHeader(
          "Content-Disposition",
          `attachment; filename="wakame.mp4"`
        );
...
app.get("/live/:id", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .render("index.html", { error: "Error fetching video info" });
  }
});
...
app.get("/listen/:id", async (req, res) => {
  ...
  if (!ytdl.validateURL(url)) {
    return res
      .status(400)
      .render("index.html", { error: "Invalid YouTube URL" });
  }
  ...
  try {
    ...
    if (audioFormats.length === 0) {
      return res
        .status(500)
        .render("index", { error: "No audio formats available" });
    }
    ...
    res
      .status(500)
      .render("index.html", { error: "Error fetching audio info" });
  }
});
...
app.get("/dd/:id", async (req, res) => {
  try {
    ...
    res.header(
      "Content-Disposition",
      `attachment; filename*=UTF-8''${encodeURIComponent(sanitizedTitle)}.mp4`
    );
...
app.get("/songs/rainbow", async (req, res) => {
  ...
  try {
    const stream_url =
      "https://cdn.glitch.me/e7208106-7973-47a2-8d4b-9fdc27b708a0/rainbow.mp4?v=1726103047477";
